...
class StoryMenuState extends MusicBeatState
{
	...
	var DemoWeekButton:FlxSprite;
	var MikeWeekButton:FlxSprite;
	...
	override function create()
	{
		...
		var bg2:FlxSprite = new FlxSprite(0., -100).loadGraphic(Paths.image('storymenu/StoryMenuUi'));
		...
		startButton = new FlxSprite(200, 600).loadGraphic(Paths.image('storymenu/Startbutton'));
		startButton.setGraphicSize(Std.int(startButton.width * 0.6));
		startButton.updateHitbox();
		add(startButton);

		DemoWeekButton = new FlxSprite(0, 0).loadGraphic(Paths.image('storymenu/Demoweek'));
		DemoWeekButton.setGraphicSize(Std.int(DemoWeekButton.width * 0.6));
		DemoWeekButton.screenCenter();
		DemoWeekButton.x += 100;
		DemoWeekButton.updateHitbox();
		add(DemoWeekButton);

		MikeWeekButton = new FlxSprite(DemoWeekButton.x + 400, 0).loadGraphic(Paths.image('storymenu/Mikeweek'));
		MikeWeekButton.setGraphicSize(Std.int(MikeWeekButton.width * 0.6));
		MikeWeekButton.screenCenter(Y);
		MikeWeekButton.updateHitbox();
		add(MikeWeekButton);

		var rankText:FlxText = new FlxText(0, 10);
		rankText.text = 'RANK: GREAT';
		rankText.setFormat(Paths.font("stranger.otf"), 32);
		...
		var tracksSprite:FlxSprite = new FlxSprite(0, 100).loadGraphic(Paths.image('Menu_Songs'));
	...
	override function update(elapsed:Float)
	{
		...
		if (!movedBack && !selectedWeek)
		{
			...
			if (FlxG.mouse.justPressed && FlxG.mouse.overlaps(startButton))
			{
				FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
				FlxFlicker.flicker(startButton, 1, 0.06, true, false, function(flick:FlxFlicker)
				{
